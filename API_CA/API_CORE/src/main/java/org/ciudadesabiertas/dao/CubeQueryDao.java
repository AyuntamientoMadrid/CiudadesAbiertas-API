 ...
 */
@Repository
public class CubeQueryDao<T> extends DatasetDao<T>
{
	
	private static final Logger log = LoggerFactory.getLogger(CubeQueryDao.class);
	
	
	
	@SuppressWarnings("unchecked")
	public  List<T> cubeQuery(String key,Class<T> type, CubeQuerySearch search, int page, int pageSize) throws DAOException, BadRequestException {
	...
	public long rowCountCubeQuery(String key,Class<T> type, CubeQuerySearch search) {
		
		log.info("[rowCountCubeQuerySearch]");
		log.debug("[rowCountCubeQuerySearch] [key:"+key+"] [type:"+type+"] [search:"+search+"]");
		int size=0;	
		Session opennedSession = null;
	...
	private String writeQuery(CubeQuerySearch search,String driver,String nameClass, String key) {
		...
		if (query.contains("*"))
		{
			log.info("[writeQuery] CubeQuery Query With *: "+query);
