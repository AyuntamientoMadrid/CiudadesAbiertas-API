...
public class DifferentSQLforDatabases {
	...
	{
	  String schema=StartVariables.sqlServerSchemas.get(key);
	...
	public static String transForm(String where, String driver) {
		...
		if (Util.getDatabaseTypeFromDriver(driver).equals(Constants.ORACLE)) {
			...
			// Esto es para no repetir extracciones, lo que ya se ha sustituido previametne
			// se cambia por --SubString--
			...
			ArrayList<Pair<String, String>> changesDate = regularExpressions
					.extractChangesDate(whereWithoutPreSubstitution);
	...
	*/
	/*
	 * Funcion que añade la funcion para eliminar acentos y no distinguir entra
	 * mayusculas y minusculas en un array con likes
 ...
 */
	...
	/*
	 * Funcion que recibe un conjunto de condiciones (where, having) y devuelve una
	 * array agrupando los likes Ejemplo: name like 'solid snake' and age>23 ->
	 * [name, like, solid snake, and, age, >, 23]
	 */
	...
	/*
	 * Metodo que sustituye los campos de la select que esten dentro de la funcion
	 * lower por la funcion translate (para borrar acentos y ñ) de cada bbdd y
	 * además pasa a mayusculas
 ...
 */
	...
	/*
	 * Metodo que sustituye los campos de la select que trata el in por la funcion
	 * translate (para borrar acentos y ñ) de cada bbdd y además pasa a mayusculas
	 * 
	...
	public static String rsqlTransFormInOut(String query, String driver, String inOrOut) throws Exception {
		...
		for (String s : split) {
			if (s.contains(inOrOut)) {
				...
				for (int i = 0; i < fieldsTranslated.size(); i++) {
					Pair<String, String> pair = new MutablePair<>("generatedAlias0." + result[i],
							fieldsTranslated.get(i));
