...
@EnableWebMvc
@EnableScheduling
@ComponentScan(basePackages = { "org.ciudadesAbiertas.madrid" })
public class WebConfig extends WebMvcConfigurerAdapter
{
	...
	for (HttpMessageConverter<?> converter : converters)
	{
		...
		StartVariables.xmlConverter = xmlMessageConverter;
	...
	if (env.getProperty("mail.to") != null)
	{
	    StartVariables.mailTo = env.getProperty("mail.to");
	}
	if (env.getProperty("mail.from") != null)
	{
	    StartVariables.mailFrom = env.getProperty("mail.from");
	}
	if (env.getProperty("mail.prefix") != null)
	{
	    StartVariables.mailPrefix = env.getProperty("mail.prefix");
	}
    }
	...
	if ("true".equals(env.getProperty("spring.mail.smtp.auth")))
	{
	    mailSender.setUsername(env.getProperty("spring.mail.username"));
	    mailSender.setPassword(Util.checkAndSetEncodedProperty(env.getProperty("spring.mail.password")));
	}
    ...
    {   
      	log.info("Tarea programada - " + new Date());
      	scheduleTask().runFromScheduler();
    }
