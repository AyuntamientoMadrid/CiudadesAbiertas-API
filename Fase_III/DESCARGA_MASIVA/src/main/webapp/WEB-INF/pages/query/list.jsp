...
<c:set var="title"
	value="<%=LiteralConstants.TITLE_HEAD_CONSULTAS_SQL%>" scope="session" />
<!DOCTYPE html>
<html lang="es">
...
<body>
		...
		<div class="brand">
			<div class="row">
				...
				<div class="col-9 options">
					<ul>
						<li><a title="Añadir" href="<c:url value="/query/add"/>"><i
								class="ico icon-add"></i><span class="text">Añadir</span></a></li>
	...
	<div class="row bloque1">
		<div class="col-xs-12 col-md-12 col-lg-10 col-xl-10">
			...
			<div class="advice">
				...
				<p><%=LiteralConstants.OPERACION_OK%>.
				</p>
			</div>
		</div>
				...
				<ul>
					<c:forEach var="entidad" items="${list}" varStatus="loop">
						...
						<h4 id="title_element_${loop.count}">
							<a href="<c:url value="/query/${entidad.queryD.code}"/>"><c:out
									value="${entidad.queryD.code}" /></a>
							<c:if test="${entidad.errorFileConfig}">
								<img
									src="<c:url value="/resources/ayre-assets/images/ico_error.png"/>"
									alt="${entidad.textoError}" />
							</c:if>
						</h4>
						<p>
							<span><c:out value="${entidad.queryD.summary}" /></span>
						</p>
						<c:if test="${entidad.nextExecution ne  ''}">
						<p  class="text-right">
							<span>Siguiente ejecución: <c:out value="${entidad.nextExecution}" /></span>
						</p>
						</c:if>
						<p class="text-right">
							<a id="linkRun${entidad.queryD.code}" style="cursor: pointer;"
								onclick="javascript:runProcess('${entidad.queryD.code}')"><i
								id='${entidad.queryD.code}Icon' class="icon icon-play"></i></a>
						</p>
						</li>
					</c:forEach>
				</ul>
	...
	<script>	
		...
		function checkProcess(code, data)
		{
			//console.log(code);
			//console.log(code.id);
			var theURL="${contextPath}/task/"+data.id;
			$.ajax({
				  url: theURL  
			})
			.done(function( data ) {			     
		...
		{
			var theURL="${contextPath}/process/"+code;
			$.ajax({
				  url: theURL,
				  beforeSend: function(  ) {
