...
import semantic.sync.util.Configuration;
import semantic.sync.util.Constants;
import semantic.sync.util.SshCommands;
import semantic.sync.util.Utils;
...
public class SemanticSyncProcess 
{
	...
	public void readConfigurationFile(String file) throws IOException {
		...
		configuration.setCatalogURL(prop.getProperty( Constants.FIELD_CATALOG_URL ));			
		configuration.setHost(prop.getProperty(Constants.FIELD_HOST));
		configuration.setUser(prop.getProperty( Constants.FIELD_USER ));
		configuration.setPassword(prop.getProperty( Constants.FIELD_PASSWORD ));

		configuration.setVirtuosoUser(prop.getProperty( Constants.FIELD_VIRTUOSO_USER ));
		configuration.setVirtuosoPassword(prop.getProperty( Constants.FIELD_VIRTUOSO_PASSWORD ));
		configuration.setVirtuosoPort(prop.getProperty( Constants.FIELD_VIRTUOSO_PORT ));
		configuration.setVirtuosoPathToIsql(prop.getProperty(  Constants.FIELD_VIRTUOSO_PATH_ISQL));
		
		configuration.setEldaURI(prop.getProperty( Constants.FIELD_VIRTUOSO_ELDA_URI ));
		
		configuration.setFormatFile(prop.getProperty( Constants.FIELD_VIRTUOSO_FORMAT_FILE ));
		
		configuration.setGraphUri(prop.getProperty( Constants.FIELD_GRAPH_URI ));
