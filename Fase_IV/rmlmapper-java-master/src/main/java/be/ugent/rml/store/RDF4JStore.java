...
public class RDF4JStore extends QuadStore {
    ...
    public RDF4JStore(List<Namespace> prefixes) {
	model = new TreeModel();
	triplesWithGraphCounter = 0;
        model.getNamespaces().addAll(prefixes);
    }
    ...
    public void write(Writer out, String format) throws Exception {
        switch (format) {
        	...
        	writerConfig.set(BasicWriterSettings.INLINE_BLANK_NODES, true);
                ...
                Rio.write(model, out, RDFFormat.TURTLE, writerConfig);                
    ...
    private Value getFilterObject(Term object) {
        if (object != null) {
            ...
            if (object instanceof BlankNode) {
                ...
                //JC (Localidata)
                if ((literal.getValue()==null)||(literal.getValue().equals("")))
                {
                  logger.debug("Error in this literal, value is null");
                  return null;
                }
              ...
              try
              {
            	String decodedURI=Utils.decodeURI(object.getValue());
            	if (decodedURI!=null)
            	{
            		return vf.createIRI(decodedURI);
            	}
            	else {
            		return null;
            	}
