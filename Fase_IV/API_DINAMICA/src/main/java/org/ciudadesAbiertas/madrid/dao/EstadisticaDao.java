...
//        result = query.list();        
	...
	public int maxRequestPerSecond_with_JPA_Criteria()
	{
	...
	public int countLastWeek() {
		...
		criteria.multiselect(builder.count(root.get("id")));
		Predicate condiciones = builder.and(				
				builder.greaterThan( root.<Date>get("fechaPeticion"), oneWeekBack )
			);
	...
	public Estadistica recent() {
		List<Estadistica> result = new ArrayList<Estadistica>();
		
		Query<Estadistica> query = sessionFactory.getCurrentSession().createQuery("FROM Estadistica order by fechaPeticion desc",Estadistica.class);
	...
	public String mostWanted() {
		...
		String hql = "select url, count(*) as c from Estadistica group by url order by c desc";
