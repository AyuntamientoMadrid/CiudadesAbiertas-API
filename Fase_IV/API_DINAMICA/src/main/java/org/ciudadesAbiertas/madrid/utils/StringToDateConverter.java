...
public class StringToDateConverter implements Converter<String, Date> {
	...
	public static Date convertStringToDate(String source)
	{
		...
		if ((theDate==null)&&(source.length()==10))
		{
			theDate=Util.getFecha(source, Constants.WEB_DATE_FORMAT);
		}
	...
	public static Time convertStringToTime(String source)
	{
		Time theTime=null;


		try {
			theTime = new java.sql.Time(Util.timeFormatter.parse(source).getTime());
		} catch (ParseException e) {
			log.error("Error parsing time: "+ source,e);
		}	
