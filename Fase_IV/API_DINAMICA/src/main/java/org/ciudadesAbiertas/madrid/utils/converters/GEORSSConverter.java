...
import org.ciudadesAbiertas.madrid.model.RDFModel;
 ...
 */
@Component
public class GEORSSConverter <T, L extends Result> extends AbstractHttpMessageConverter<L> {
	...
	private SyndEntry transformGeoRSS(T record, String petitionSrId) throws Exception {
		...
		for (BeanUtil bean:listData)
		{
			...
			{
				if (bean.getFieldName().equals(StartVariables.xETRS89_field))
				{	
					lat = checkStringOrBigDecimal(bean);
				}
				else if (bean.getFieldName().equals(StartVariables.yETRS89_field))
				{
					lon= checkStringOrBigDecimal(bean);
				}
			}else {
	...
	private BigDecimal checkStringOrBigDecimal(BeanUtil bean) {
		BigDecimal lat;
	...
	{	
		...
		{			
			...
			if (bean.getFieldName().equals(StartVariables.geometry_field))
			{				
				JSONObject hasGeometry=Util.stringToJSONObject((String)bean.getValue());
				if (hasGeometry!=null)
				{
	...
	private void addFieldOdata(BeanUtil bean, List <Element> listaElementos) throws Exception {
		...
		else if (bean.getValue() instanceof String)
		{
		...
		else if (bean.getTypeName().equals("java.lang.Boolean")||(bean.getTypeName().equals("boolean")))
		{
			type="Edm.Boolean";			
		}
		...
		else if (bean.getTypeName().equals("org.json.simple.JSONObject")&&(bean.getFieldName().equals("hasGeometry")))
		{
			stringValue=(String) bean.getValue().toString();
			type="Edm.String";
		}
